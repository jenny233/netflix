...
double REGULARIZATION_Y   = 0.02;
double LEARNING_RATE      = 0.005;
double LEARNING_RATE_Y    = 0.001;
double LEARNING_RATE_BIAS = 0.001;
int    MAX_EPOCH          = 400;
string PRED_FILENAME = "../probe_predictions_svd++_"+to_string(RUN_NUMBER)+"_"+to_string(LATENT_FACTORS)+"lf_";
			...
			double* movie_bias, int* user_matrix_test, short* movie_matrix_test, short* date_matrix_test)
{
    ...
    for (long r=0; r<TEST_SIZE; r++) {
		...
		double prediction = predict_score( U, V, SumMW, i, j, user_bias[i], movie_bias[j], sqrt_r);
