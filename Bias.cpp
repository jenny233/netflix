...
double SGD_bias_u(double u_bias[], double m_bias[], int* rating_matrix[], double lamdba, double avg, int rating_index, double learning_rate)
...
double SGD_bias_m(double u_bias[], double m_bias[], int* rating_matrix[], double lamdba, double avg, int rating_index,double learning_rate)
...
void gradient_descent_bias(double u_bias[], double m_bias[], int* rating_matrix[], double lamdba, double avg)
{
	...
	while (abs(error - new_error)/ error > (.000001))
	{
		...
		for (int j = 0; j < USER_SIZE; j++)
		{
			...
			u_bias[rating_matrix[j][0]] = SGD_bias_u(u_bias, m_bias, rating_matrix, lamdba, avg, j, learning_rate);
			m_bias[rating_matrix[j][1]] = SGD_bias_m(u_bias, m_bias, rating_matrix, lamdba, avg, j, learning_rate);
...

{
	...
	array_pop(READ_IN_LINES, rating_matrix, adj_user, 'u');

	array_pop(READ_IN_LINES, rating_matrix, adj_movie, 'm');
